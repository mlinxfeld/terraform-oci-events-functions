resource "null_resource" "FoggyKitchenFnSetup" {
  ...
  provisioner "local-exec" {
    command = "image=$(docker images | grep updatetagforbootvolumebackupfn | awk -F ' ' '{print $3}') ; docker tag $image ${var.ocir_docker_repository}/${var.ocir_namespace}/${var.ocir_repo_name}/updatetagforbootvolumebackupfn:0.0.1"
    working_dir = "functions/UpdateTagForBootVolumeBackupFn"
  }
  ...
  provisioner "local-exec" {
    command = "image=$(docker images | grep updatetagforvolumebackupfn | awk -F ' ' '{print $3}') ; docker tag $image ${var.ocir_docker_repository}/${var.ocir_namespace}/${var.ocir_repo_name}/updatetagforvolumebackupfn:0.0.1"
    working_dir = "functions/UpdateTagForVolumeBackupFn"
  }
